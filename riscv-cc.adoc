...
This section applies only to named arguments. Variadic arguments are passed according to the integer calling convention.

The hardware vector calling convention adds 1 argument register of vector data type and 31 argument registers of vector data type which are v0 and v1-v31, respectively. v0 is used for the first vector mask type argument and the vector mask type return value, the rest of the mask type arguments are treated as vector data type arguments. v1-v31 are also used for the vector data type return value.

Vector data type arguments have property LMUL, the current LMUL can be 1/8, 1/4, 1/2, 1, 2, 4, 8. For arguments with LMUL less than 1, their LMUL is treated as 1. The LMUL of the vector mask type argument is treated as 1. If it is possible to find unused continuous vector register set starting from v1 and the first register in segment is LMUL-aligned, use these registers to pass the argument. Otherwise, pass through the function stack.

A struct containing just one vector type value is passed as though it were a standalone vector type value.

A struct containing two vector type values is passed in vector registers, if there are suitable unused continuous vector register sets for them. Otherwise, it is passed according to the integer calling convention.
...
A struct is not passed in this manner, then it is passed according to the integer calling convention.
...
Struct or vector scalars passed on the stack are aligned to the type alignment.
